...
def powell(x,y,e,f,sun_dlp,x_t,y_t,width,height,x_3_e):
    R = 1737.4
    num = np.size(x)
    num_t = np.size(x_t)
    if num != np.size(y):
    ...
    def objective_function(a):
        ...
        # print(x_3)
        # print(x_3_e)
        # return np.linalg.norm(x_3 - x_3_e) + np.dot(a, np.dot(D.T, np.dot(D, a))) 
    ...
    result = minimize(objective_function, a0, method='Powell')
