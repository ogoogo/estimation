...
% limited = 0;
sets = ["fitz","fitz_zernike"]; 
colors = ["b","r"];
legends = ["fitzgibbon", "fitzgibbon with zernike"];
...
plot(mvmeans(1,:),mvmeans(2,:),'LineWidth',2)
hold on 
plot(mvmeans(1,:),mvmeans(3,:),'LineWidth',2)
...
mvmeans = zeros(3,20);
for i = 1:20
    rows1 = data1(3,:) >= (i-1)*50000 & data1(3,:) <= i*50000;
    rows2 = data2(3,:) >= (i-1)*50000 & data2(3,:) <= i*50000;
    mvmeans(1,i) = (i-0.5)*50000;
    mvmeans(2,i) = mean(data1(2,rows1));
    mvmeans(3,i) = mean(data2(2,rows2));
end
plot(mvmeans(1,:),mvmeans(2,:),'LineWidth',2)
